...
export default function FeatureCards() {
  ...
  const FloatingIcons = () => (
    <div className="absolute inset-[1rem] grid grid-cols-5 gap-x-3 gap-y-1 scale-105 z-10">
      {providerIcons.slice(0, 25).map((icon, index) => (
        <div 
          key={index}
          ...
          style={{
            ...
            opacity: 0.85,
            transform: index > 4 && index < 10 ? 'translateX(36px)' : 
                     index >= 15 && index < 20 ? 'translateX(36px)' : 'translateX(0)'
          }}
        >
          <div className="flex items-center justify-center size-6 flex-shrink-0 rounded-full border border-border/50 shadow bg-muted p-0.5">
            <div className="overflow-hidden rounded-full">
              <img 
                ...
                className={`h-full w-full object-cover ${icon.includes('OpenAI') ? 'dark:invert' : ''}`}
                src={icon}
              />
            </div>
          </div>
    ...
    <div className="space-y-4 md:space-y-8">
      <div className="grid md:grid-cols-2 gap-6 lg:grid-cols-4">
        {features.map((feature, index) => (
          <a key={feature.title} className="h-full" href={feature.link}>
            <div className="group/card text-card-foreground rounded-xl transition-all duration-200 bg-card hover:border-slate-300 hover:text-slate-800 border shadow-sm hover:shadow-lg flex flex-col h-full justify-between overflow-hidden">
              ...
              <div className="flex flex-col gap-2 px-6 py-4 h-full">
                <div className="flex flex-col gap-2 h-full">
                  <h3 className="group-hover/card:text-slate-800 transition-colors duration-200 text-2xl font-semibold">
                    {feature.title}
                  </h3>
